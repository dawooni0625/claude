...
This folder is a design system. It contains the canonical CSS tokens, a Korean-first type stack, a clickable admin UI kit, and ~33 preview cards demonstrating colour, type, spacing, and component patterns.
...
| `tokens.css` | Every CSS custom property (light + dark). The runtime source of truth. |
| `colors_and_type.css` | Webfont imports, base reset, semantic text classes. |
| `tokens/*.json` | W3C-DTCG source (18 categories) + upstream Korean docs. |
| `preview/` | 33 specimen cards — colours, type scale, spacing, components, brand. Copy-paste-friendly HTML. |
| `icon/` | Full 113-icon set, 10 categories (social: filled + line). `currentColor`. |
| `ui_kits/admin/` | Clickable React UI kit. Sidebar shell, login, dashboard, members table, settings form. |
